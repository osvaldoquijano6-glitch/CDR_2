...
def generar_plan_trabajo_cre_docx(inst: Installation, path: Path) -> Path:
    """Plan de Trabajo CRE para Centros de Carga (formato del Cap. 4.1, Manual CONE).

    Documento a presentar ante la CRE. Reproduce la estructura oficial con campos
    en blanco para captura; prellenados los datos que la instalación ya aporta
    (nivel de tensión, demanda contratada) y las tablas de requerimientos técnicos
    con los rangos obligatorios. Sin nombres de empresa: placeholders.
    """
    from docx import Document
    from docx.enum.text import WD_ALIGN_PARAGRAPH
    from docx.shared import Pt, RGBColor
...
def generar_paquete(inst: Installation, pruebas_ids: list[str], outdir: Path) -> dict[str, Path]:
    ...
    if inst.kind.value == "CENTRO_DE_CARGA":
        paquete["plan_trabajo"] = generar_plan_trabajo_cre_docx(
            inst, outdir / f"{slug}_Plan_de_Trabajo_CRE.docx")
    return paquete
